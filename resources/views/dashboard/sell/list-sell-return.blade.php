...
<div class="page-card">
  ...
  <div style="overflow-x:auto;">
    <table class="tbl">
      <thead><tr><th>#</th><th>Return Ref</th><th>Customer</th><th>Date</th><th>Amount</th><th>Reason</th><th>Actions</th></tr></thead>
...
<div class="modal-overlay" id="modal">
  <div class="modal modal-lg">
    <div class="modal-header">
      <div class="modal-title" id="modal-title">New Sales Return</div>
      ...
      <form id="itemForm">
        <div class="form-row">
          <div class="form-group">
            ...
            <select name="customer_id" class="form-control" id="customerSel">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input name="total" type="number" step="0.01" min="0" class="form-control" required placeholder="0.00" oninput="syncSubtotal(this)">
            <div class="invalid-feedback"></div>
          </div>
          <div class="form-group">
...
@section('scripts')
<script>
const API='/api/dashboard/sales'; let editId=null;
function syncSubtotal(el){document.getElementById('subtotalHidden').value=el.value;}
async function loadList(){
  ...
  try{
    ...
    if(!items.length){tbody.innerHTML='<tr><td colspan="7" class="tbl-empty">No returns found.</td></tr>';return;}
    tbody.innerHTML=items.map((r,i)=>`<tr>
      ...
      <td class="text-slate-500 text-xs">${r.sale_date}</td>
...
async function openAddModal(){
  editId=null;
  document.getElementById('modal-title').textContent='New Sales Return';
  document.getElementById('itemForm').reset();
  const today=new Date().toISOString().split('T')[0];
  document.querySelector('[name="sale_date"]').value=today;
  try{
    const customers=await apiFetch('/api/dashboard/customers');
    document.getElementById('customerSel').innerHTML='<option value="">Walk-in Customer</option>'+customers.map(c=>`<option value="${c.id}">${c.name}</option>`).join('');
  }catch(e){}
...
async function saveItem(){
  ...
  data.items=[{product_name:'Return',quantity:1,unit_price:parseFloat(data.total||0),discount:0,total:parseFloat(data.total||0)}];
  data.subtotal=parseFloat(data.total||0);
  const btn=document.getElementById('saveBtn');btn.disabled=true;btn.textContent='Saving...';
  try{
    await apiFetch(API,{method:'POST',body:JSON.stringify(data)});
    closeModal('modal');showToast('Return recorded!');loadList();
