...
<div class="page-card">
  ...
  <div style="overflow-x:auto;">
    <table class="tbl">
      <thead><tr><th>#</th><th>Reference</th><th>Customer</th><th>Date</th><th>Total</th><th>Payment</th><th>Actions</th></tr></thead>
...
<div class="modal-overlay" id="viewModal">
  <div class="modal modal-lg">
    <div class="modal-header">
      <div class="modal-title">Quotation Details</div>
...
async function loadList(){
  ...
  try{
    const items=await apiFetch(`${API}?status=quotation&search=${encodeURIComponent(s)}`);
    if(!items.length){tbody.innerHTML='<tr><td colspan="7" class="tbl-empty">No quotations found.</td></tr>';return;}
    tbody.innerHTML=items.map((s,i)=>`<tr>
      ...
      <td><div style="display:flex;gap:0.4rem;">
        <button class="btn btn-sm btn-edit btn-icon" onclick="viewQuote(${s.id})" title="View"><svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></button>
        <button class="btn btn-sm btn-delete btn-icon" onclick="deleteQuote(${s.id},'${s.reference}')"><svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
      </div></td>
    </tr>`).join('');
...
async function viewQuote(id){
  ...
  try{
    ...
    openModal('viewModal');
  }catch(e){showToast('Failed to load quotation','error');}
}
async function convertToSale(){
  ...
  try{
    ...
    closeModal('viewModal');showToast('Quotation converted to sale!');loadList();
  }catch(e){showToast(e.message||'Failed to convert','error');}
  finally{btn.disabled=false;btn.textContent='Convert to Sale';}
}
function deleteQuote(id,ref){
  showConfirm('Delete Quotation',`Delete quotation "${ref}"?`,async()=>{
    try{await apiFetch(`${API}/${id}`,{method:'DELETE'});showToast('Quotation deleted!');loadList();}
