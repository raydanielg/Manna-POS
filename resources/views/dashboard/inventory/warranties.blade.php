...
@section('page_title','Warranties')
@section('content')
<div class="dash-content">
<div class="page-card">
  <div class="card-header">
    <div class="card-title">Warranties</div>
  ...
  <div style="overflow-x:auto;">
    <table class="tbl">
      <thead><tr><th>#</th><th>Name</th><th>Duration</th><th>Description</th><th>Actions</th></tr></thead>
...
<div class="modal-overlay" id="modal">
  <div class="modal">
    <div class="modal-header">
      <div class="modal-title" id="modal-title">Add Warranty</div>
    ...
    <div class="modal-body">
      <form id="itemForm">
        <div class="form-group"><label class="form-label">Warranty Name *</label><input name="name" class="form-control" required placeholder="e.g. 1 Year Manufacturer Warranty"><div class="invalid-feedback"></div></div>
        <div class="form-row">
          <div class="form-group"><label class="form-label">Duration *</label><input name="duration" type="number" min="1" class="form-control" required placeholder="e.g. 12"><div class="invalid-feedback"></div></div>
        ...
        <div class="form-group"><label class="form-label">Description</label><textarea name="description" class="form-control" rows="3" placeholder="Warranty details..."></textarea><div class="invalid-feedback"></div></div>
      </form>
    </div>
...
@section('scripts')
<script>
const API='/api/dashboard/warranties'; let editId=null;
async function loadList(){
  ...
  try{
    const items=await apiFetch(`${API}?search=${encodeURIComponent(s)}${unit?'&duration_unit='+unit:''}`);
    if(!items.length){tbody.innerHTML='<tr><td colspan="5" class="tbl-empty">No warranties found.</td></tr>';return;}
    tbody.innerHTML=items.map((w,i)=>`<tr>
      <td class="text-slate-400">${i+1}</td>
      <td class="font-semibold">${w.name}</td>
      <td><span class="badge badge-info">${w.duration} ${w.duration_unit}</span></td>
      <td class="text-slate-500">${w.description||'-'}</td>
      <td><div style="display:flex;gap:0.4rem;">
        <button class="btn btn-sm btn-edit btn-icon" onclick="editItem(${w.id})"><svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
        <button class="btn btn-sm btn-delete btn-icon" onclick="deleteItem(${w.id},'${w.name}')"><svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
      </div></td>
    </tr>`).join('');
  }catch(e){tbody.innerHTML='<tr><td colspan="5" class="tbl-empty">Error loading data.</td></tr>';}
}
function openAddModal(){editId=null;document.getElementById('modal-title').textContent='Add Warranty';document.getElementById('itemForm').reset();clearFormErrors('itemForm');openModal('modal');}
async function editItem(id){
  try{const w=await apiFetch(`${API}/${id}`);editId=id;document.getElementById('modal-title').textContent='Edit Warranty';
  const form=document.getElementById('itemForm');Object.entries(w).forEach(([k,v])=>{const el=form.querySelector(`[name="${k}"]`);if(el)el.value=v??'';});
...
async function saveItem(){
  ...
  closeModal('modal');showToast(editId?'Warranty updated!':'Warranty added!');loadList();}
  catch(e){if(e.errors)showFormErrors('itemForm',e.errors);else showToast(e.message||'Save failed','error');}
  finally{btn.disabled=false;btn.textContent='Save Warranty';}
}
function deleteItem(id,name){
  showConfirm('Delete Warranty',`Delete warranty "${name}"?`,async()=>{
    try{await apiFetch(`${API}/${id}`,{method:'DELETE'});showToast('Warranty deleted!');loadList();}
...
async function openImportModal(){
  ...
  try{
    ...
    list.innerHTML=items.map((it)=>`<label style="display:flex;align-items:center;gap:0.75rem;padding:0.5rem;border-radius:6px;cursor:pointer;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
      ...
      <div style="flex:1;"><div class="font-semibold">${it.name}</div><div class="text-xs text-slate-500">${it.duration} ${it.duration_unit} - ${it.description||''}</div></div>
    </label>`).join('');
...
async function importSelected(){
  ...
  try{
    ...
    closeModal('importModal');showToast(res.message||`Imported ${res.imported} warranties`);
    loadList();
