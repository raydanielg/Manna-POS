...
@section('page_title','Import Opening Stock')
@section('content')
<div class="dash-content">
<div class="page-card" style="max-width:860px;">
  <div class="card-header"><div class="card-title">Import Opening Stock</div></div>
  <div class="modal-body">
    <div style="background:#fef3c7;border:1px solid #fde68a;border-radius:10px;padding:1rem 1.25rem;margin-bottom:1.5rem;">
      <div style="font-size:0.85rem;font-weight:700;color:#d97706;margin-bottom:0.5rem;">Important Note</div>
      <div style="font-size:0.82rem;color:#1e293b;">This imports stock as "addition" adjustments. Each row will create a stock adjustment record and increase the product's stock quantity.</div>
    </div>
    <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:1rem 1.25rem;margin-bottom:1.5rem;">
      <div style="font-size:0.85rem;font-weight:700;color:#1d4ed8;margin-bottom:0.5rem;">CSV Format</div>
      <div style="font-size:0.82rem;color:#1e293b;">Required: <code>product_id, quantity</code></div>
      <div style="font-size:0.82rem;color:#64748b;margin-top:0.25rem;">Optional: <code>unit_cost, reason</code></div>
    ...
    <div id="previewSection" style="display:none;margin-top:1.5rem;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.75rem;">
        ...
        <div style="display:flex;gap:0.5rem;">
          ...
          <button class="btn btn-success" id="importBtn" onclick="importData()">Import Stock</button>
        </div>
      </div>
...
function downloadTemplate(){
  const csv='product_id,quantity,unit_cost,reason\n1,100,5.00,Opening stock\n2,50,10.00,Initial inventory';
  const blob=new Blob([csv],{type:'text/csv'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='opening_stock_template.csv';a.click();
}
...
function parseCSV(text){
  ...
  document.getElementById('previewTitle').textContent=`Preview: ${parsedData.length} adjustment(s) to create`;
  document.getElementById('previewTable').innerHTML=`<thead><tr>${headers.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${parsedData.slice(0,10).map(r=>`<tr>${headers.map(h=>`<td>${r[h]||'-'}</td>`).join('')}</tr>`).join('')}${parsedData.length>10?`<tr><td colspan="${headers.length}" class="tbl-empty text-xs">... and ${parsedData.length-10} more</td></tr>`:''}</tbody>`;
...
async function importData(){
  ...
  for(const row of parsedData){
    if(!row.product_id||!row.quantity){failed++;continue;}
    try{await apiFetch('/api/dashboard/stock-adjustments',{method:'POST',body:JSON.stringify({product_id:parseInt(row.product_id),quantity:parseFloat(row.quantity),type:'addition',unit_cost:parseFloat(row.unit_cost)||null,reason:row.reason||'Opening stock import',adjustment_date:today})});success++;}
    catch(e){failed++;}
  ...
  btn.disabled=false;btn.textContent='Import Stock';
  if(failed===0)showToast(`Successfully imported ${success} stock adjustments!`);
