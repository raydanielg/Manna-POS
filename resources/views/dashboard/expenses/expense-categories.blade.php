...
<div class="page-card">
  ...
  <div style="overflow-x:auto;">
    <table class="tbl">
      <thead><tr><th>#</th><th>Category Name</th><th>Expenses</th><th>Description</th><th>Actions</th></tr></thead>
...
<div class="modal-overlay" id="modal">
  <div class="modal">
    <div class="modal-header">
      <div class="modal-title" id="modal-title">Add Expense Category</div>
    ...
    <div class="modal-body">
      <form id="itemForm">
        <div class="form-group"><label class="form-label">Category Name *</label><input name="name" class="form-control" required placeholder="e.g. Rent, Utilities, Salaries"><div class="invalid-feedback"></div></div>
        <div class="form-group"><label class="form-label">Description</label><textarea name="description" class="form-control" placeholder="Category description..."></textarea><div class="invalid-feedback"></div></div>
      </form>
    </div>
...
@section('scripts')
<script>
const API='/api/dashboard/expense-categories'; let editId=null;
async function loadList(){
  ...
  try{
    ...
    if(!items.length){tbody.innerHTML='<tr><td colspan="5" class="tbl-empty">No categories found.</td></tr>';return;}
    tbody.innerHTML=items.map((c,i)=>`<tr>
      <td class="text-slate-400">${i+1}</td><td class="font-semibold">${c.name}</td>
      <td><span class="badge badge-info">${c.expenses_count||0}</span></td>
      <td class="text-slate-400">${c.description||'—'}</td>
...
function openAddModal(){editId=null;document.getElementById('modal-title').textContent='Add Expense Category';document.getElementById('itemForm').reset();clearFormErrors('itemForm');openModal('modal');}
async function editItem(id){
  try{const c=await apiFetch(`${API}/${id}`);editId=id;document.getElementById('modal-title').textContent='Edit Expense Category';
