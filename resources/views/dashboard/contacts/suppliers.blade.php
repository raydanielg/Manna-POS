...
<div class="page-card">
  <div class="card-header">
    ...
    <div class="filters-row">
      <div class="search-wrap">
        ...
        <input type="text" id="searchInput" placeholder="Search suppliers..." oninput="loadList()">
      </div>
  ...
  <div style="overflow-x:auto;">
    <table class="tbl">
      <thead><tr><th>#</th><th>Name</th><th>Company</th><th>Email</th><th>Phone</th><th>City</th><th>Balance</th><th>Status</th><th>Actions</th></tr></thead>
...
<div class="modal-overlay" id="modal">
  <div class="modal">
      ...
      <form id="supplierForm">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <input name="country" class="form-control" value="Tanzania" placeholder="Country">
            <div class="invalid-feedback"></div>
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
            ...
            <input name="tax_number" class="form-control" placeholder="TIN / VAT number">
            <div class="invalid-feedback"></div>
          </div>
          <div class="form-group">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input name="pay_term" class="form-control" placeholder="e.g. Net 30">
            <div class="invalid-feedback"></div>
          </div>
          <div class="form-group">
    ...
    <div class="modal-footer">
      ...
      <button class="btn btn-primary" id="saveBtn" onclick="saveSupplier()">Save Supplier</button>
    </div>
  </div>
</div>
@endsection
...
@section('scripts')
<script>
const API = '/api/dashboard/suppliers';
let editId = null;
...
async function loadList() {
  ...
  try {
    ...
    if (!items.length) { tbody.innerHTML = '<tr><td colspan="9" class="tbl-empty">No suppliers found.</td></tr>'; return; }
    tbody.innerHTML = items.map((s,i) => `
      <tr>
        ...
        <td>${s.phone||'—'}</td>
        <td>${s.city||'—'}</td>
        <td>TSh ${Number(s.balance||0).toLocaleString()}</td>
        <td><span class="badge ${s.status==='active'?'badge-success':'badge-danger'}">${s.status}</span></td>
        <td>
          <div style="display:flex;gap:0.4rem;">
            <button class="btn btn-sm btn-edit btn-icon" onclick="editSupplier(${s.id})" title="Edit">
            ...
            <button class="btn btn-sm btn-delete btn-icon" onclick="deleteSupplier(${s.id},'${s.name}')" title="Delete">
...
async function editSupplier(id) {
  try {
    ...
    Object.entries(s).forEach(([k,v]) => { const el = form.querySelector(`[name="${k}"]`); if (el) el.value = v??''; });
    clearFormErrors('supplierForm');
    openModal('modal');
  } catch(e) { showToast('Failed to load supplier','error'); }
}
...
async function saveSupplier() {
  ...
  try {
    ...
    if (e.errors) showFormErrors('supplierForm', e.errors);
    else showToast(e.message||'Save failed','error');
  } finally { btn.disabled = false; btn.textContent = 'Save Supplier'; }
}

function deleteSupplier(id, name) {
  showConfirm('Delete Supplier', `Delete "${name}"? This cannot be undone.`, async () => {
    try { await apiFetch(`${API}/${id}`, {method:'DELETE'}); showToast('Supplier deleted!'); loadList(); }
