...
class AdminDemoSeeder extends Seeder
{
    public function run()
    {
        ...
        $plans = [
            ['name' => 'Starter', 'slug' => 'starter', 'description' => 'Perfect for small shops just getting started with digital POS.', 'price_monthly' => 0, 'price_yearly' => 0, 'currency' => 'TZS', 'max_users' => 1, 'max_products' => 100, 'max_locations' => 1,                 'features' => json_encode(['Basic Sales', 'Inventory (up to 100 products)', 'Cash Payments', 'Daily Reports']), 'is_active' => true, 'is_featured' => false, 'sort_order' => 1, 'badge_color' => '#94a3b8'],
            ['name' => 'Growth', 'slug' => 'growth', 'description' => 'For growing businesses that need inventory management and reports.', 'price_monthly' => 45000, 'price_yearly' => 450000, 'currency' => 'TZS', 'max_users' => 3, 'max_products' => 1000, 'max_locations' => 2, 'features' => json_encode(['Everything in Starter', 'Up to 3 Users', 'Inventory (1,000 products)', 'Mobile Money Payments', 'Customer Profiles', 'Sales Analytics']), 'is_active' => true, 'is_featured' => true, 'sort_order' => 2, 'badge_color' => '#2563eb'],
            ['name' => 'Business', 'slug' => 'business', 'description' => 'For established businesses with multiple staff and locations.', 'price_monthly' => 95000, 'price_yearly' => 950000, 'currency' => 'TZS', 'max_users' => 10, 'max_products' => 10000, 'max_locations' => 5, 'features' => json_encode(['Everything in Growth', 'Up to 10 Users', 'Multi-Location (5 branches)', 'Staff Management', 'Purchase Orders', 'Barcode Scanning', 'Advanced Reports']), 'is_active' => true, 'is_featured' => true, 'sort_order' => 3, 'badge_color' => '#16a34a'],
            ['name' => 'Enterprise', 'slug' => 'enterprise', 'description' => 'For large organizations with advanced needs and dedicated support.', 'price_monthly' => 250000, 'price_yearly' => 2500000, 'currency' => 'TZS', 'max_users' => 50, 'max_products' => 999999, 'max_locations' => 50, 'features' => json_encode(['Everything in Business', 'Unlimited Products', 'Up to 50 Users', '50 Locations', 'API Access', 'Dedicated Account Manager', 'Priority Support', 'Custom Integrations']), 'is_active' => true, 'is_featured' => false, 'sort_order' => 4, 'badge_color' => '#7c3aed'],
        ];
        foreach ($plans as $p) {
        ...
        SmsTemplate::insert([
            ['name' => 'Welcome SMS', 'code' => 'welcome_sms', 'message' => 'Welcome {{name}} to MannaPOS! Your account is ready. Start selling today!', 'variables' => ['name'], 'category' => 'onboarding', 'is_active' => true],
            ['name' => 'Payment Confirmation', 'code' => 'payment_confirm', 'message' => 'Your payment of {{amount}} {{currency}} for invoice #{{invoice}} has been received. Thank you!', 'variables' => ['amount', 'currency', 'invoice'], 'category' => 'billing', 'is_active' => true],
            ['name' => 'Low Stock Alert', 'code' => 'low_stock', 'message' => 'ALERT: {{product}} is low on stock ({{qty}} remaining). Please restock soon.', 'variables' => ['product', 'qty'], 'category' => 'inventory', 'is_active' => true],
            ['name' => 'Subscription Renewed', 'code' => 'sub_renewed', 'message' => 'Your {{plan}} subscription has been renewed. Valid until {{expiry}}. Thank you for being a valued customer!', 'variables' => ['plan', 'expiry'], 'category' => 'billing', 'is_active' => false],
        ]);
        ...
        Backup::insert([
            ...
            ['name' => 'Daily Backup - June 14', 'file_path' => 'backups/daily-2025-06-14.zip', 'type' => 'daily', 'size' => 127500000, 'status' => 'completed', 'created_by' => 2],
        ]);
