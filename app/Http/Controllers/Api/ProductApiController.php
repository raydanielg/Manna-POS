...
class ProductApiController extends Controller {
    public function index(Request $req) {
        $q = Product::with(['category','brand','unit']);
        if ($req->search) $q->where('name','like','%'.$req->search.'%')->orWhere('sku','like','%'.$req->search.'%')->orWhere('barcode','like','%'.$req->search.'%');
        if ($req->category_id) $q->where('product_category_id',$req->category_id);
        if ($req->status) $q->where('status',$req->status);
        $products = $q->orderBy('name')->get();
        return response()->json($products->map(fn($p) => $this->productArr($p)));
    }
    ...
    public function show($id) {
        $p = Product::with(['category','brand','unit'])->findOrFail($id);
        return response()->json($this->productArr($p));
    }
