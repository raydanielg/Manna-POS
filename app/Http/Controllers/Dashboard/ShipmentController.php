...
class ShipmentController extends Controller {
    public function index(Request $req) {
        $q = Shipment::query();
        if ($req->search) $q->where('reference','like',"%{$req->search}%")->orWhere('recipient_name','like',"%{$req->search}%")->orWhere('tracking_number','like',"%{$req->search}%");
    ...
    public function show(Shipment $shipment) { return response()->json($shipment); }
    ...
    public function destroy(Shipment $shipment) { $shipment->delete(); return response()->json(['success'=>true]); }
}
