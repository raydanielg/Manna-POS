...
class ProductController extends Controller {
    public function index(Request $req) {
        $q = Product::with(["brand","category","unit"])->forCurrentUser($this->currentBusinessId());
    ...
    public function create() {
        return view('dashboard.inventory.list-products', [
            'categories' => ProductCategory::forCurrentUser($this->currentBusinessId())->get(),
            'brands' => Brand::forCurrentUser($this->currentBusinessId())->get(),
            'units' => Unit::forCurrentUser($this->currentBusinessId())->get(),
            'taxRates' => TaxRate::forCurrentUser($this->currentBusinessId())->get(),
        ]);
    }
    ...
    public function show(Product $product) { $this->ensureOwns($product); return response()->json($product); }
    public function update(Request $req, Product $product) {
        $this->ensureOwns($product);
    ...
    public function destroy(Product $product) { $this->ensureOwns($product); $product->delete(); return response()->json(["success"=>true]); }
}
