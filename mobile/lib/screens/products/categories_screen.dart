...
class _CategoriesScreenState extends State<CategoriesScreen> {
  ...
  void _showForm([Map<String, dynamic>? category]) {
    AppBottomSheet.show(
      ...
      child: _CategoryForm(category: category, onSaved: _refresh),
    );
  }
  ...
  Future<void> _delete(Map<String, dynamic> c) async {
    ...
    try {
      await context.read<ProductProvider>().deleteCategory(c['id']);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Categories'), actions: [IconButton(icon: const Icon(Icons.refresh), onPressed: _refresh)]),
      body: Consumer<ProductProvider>(
        builder: (context, provider, _) {
          if (provider.isLoading) return const ShimmerLoading();
          if (provider.categories.isEmpty) {
            return const EmptyState(icon: Icons.category_outlined, title: 'No Categories', subtitle: 'Add your first category', actionLabel: 'Add Category');
          }
            ...
            child: ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                return Dismissible(
                  ...
                  confirmDismiss: (_) async { await _delete(c.toJson()); return false; },
                  child: GlassCard(
                    onTap: () => _showForm(c.toJson()),
                    child: ListTile(
                      leading: Container(width: 14, height: 14, decoration: BoxDecoration(color: c.color ?? AppColors.primary, shape: BoxShape.circle)),
                      title: Text(c.name, style: const TextStyle(fontWeight: FontWeight.w600)),
                      subtitle: c.description != null ? Text(c.description!, style: const TextStyle(fontSize: 11)) : null,
                      trailing: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Text('${c.productCount} products', style: const TextStyle(color: AppColors.textSec, fontSize: 12)),
...
class _CategoryFormState extends State<_CategoryForm> {
  ...
  final _colors = [
    AppColors.primary, AppColors.secondary, AppColors.success, AppColors.warning,
  ...
  Future<void> _save() async {
    ...
    try {
      if (widget.category != null) {
        await context.read<ProductProvider>().updateCategory(widget.category!['id'], body);
      } else {
        await context.read<ProductProvider>().createCategory(body);
      }
      widget.onSaved();
