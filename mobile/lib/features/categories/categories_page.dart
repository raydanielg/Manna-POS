...
class _CategoriesPageState extends State<CategoriesPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await ApiService.get('/product-categories');
  ...
  void _showForm([dynamic c]) => showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (_) => _CategoryForm(category: c, onSaved: _load));
  ...
  Future<void> _delete(dynamic c) async {
    ...
    if (ok != true) return;
    try { await ApiService.delete('/product-categories/${c['id']}'); _load(); } on ApiException catch (e) { _snack(e.message, error: true); }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _loading ? const LoadingWidget(message: 'Loading...')
        ...
        : _categories.isEmpty ? const EmptyState(icon: Icons.category_outlined, title: 'No Categories', subtitle: 'Add your first category')
  ...
  Widget _tile(dynamic c) => AppCard(child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
    ...
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (c['parent'] != null) Text('Parent: ${c['parent']['name']}', style: const TextStyle(color: AppColors.textSec, fontSize: 12)),
    ])),
    Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
      StatusBadge.fromStatus(c['status'] ?? 'active'),
...
class _CategoryForm extends StatefulWidget {
  ...
  @override State<_CategoryForm> createState() => _CategoryFormState();
}
...
class _CategoryFormState extends State<_CategoryForm> {
  ...
  Future<void> _loadParents() async {
    try {
      final data = await ApiService.get('/product-categories');
      setState(() { _parents = (data as List).where((c) => c['id'] != (widget.category?['id'])).toList(); });
    } catch (_) {}
  }
  ...
  Future<void> _save() async {
    ...
    final body = {'name': _name.text.trim(), if (_desc.text.isNotEmpty) 'description': _desc.text.trim(), 'parent_id': _parentId};
    try {
      if (widget.category != null) await ApiService.put('/product-categories/${widget.category!['id']}', body);
      else await ApiService.post('/product-categories', body);
      widget.onSaved();
