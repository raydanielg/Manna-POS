...
class ExpenseCategoriesPage extends StatefulWidget {
  ...
  @override State<ExpenseCategoriesPage> createState() => _ExpenseCategoriesPageState();
}
...
class _ExpenseCategoriesPageState extends State<ExpenseCategoriesPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await ApiService.get('/expense-categories');
  ...
  void _showForm([dynamic c]) => showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (_) => _Form(category: c, onSaved: _load));
  ...
  Future<void> _delete(dynamic c) async {
    ...
    if (ok != true) return;
    try { await ApiService.delete('/expense-categories/${c['id']}'); _load(); } on ApiException catch (e) { _snack(e.message, error: true); }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Expense Categories'), actions: [IconButton(icon: const Icon(Icons.refresh, color: Colors.white), onPressed: _load)]),
      body: _loading ? const LoadingWidget(message: 'Loading...')
        : _error != null ? ErrorWidget2(message: _error!, onRetry: _load)
        : _categories.isEmpty ? const EmptyState(icon: Icons.category_outlined, title: 'No Categories', subtitle: 'Add your first expense category')
        : RefreshIndicator(color: AppColors.primary, onRefresh: _load,
            child: ListView.separated(padding: const EdgeInsets.fromLTRB(16, 0, 16, 100), itemCount: _categories.length, separatorBuilder: (_, __) => const SizedBox(height: 10), itemBuilder: (_, i) => _tile(_categories[i]))),
  ...
  Widget _tile(dynamic c) => AppCard(child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
    Container(width: 46, height: 46, decoration: BoxDecoration(color: AppColors.secondaryLt, borderRadius: BorderRadius.circular(12)),
    ...
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (c['description'] != null) Text(c['description'], style: const TextStyle(color: AppColors.textSec, fontSize: 12), maxLines: 1, overflow: TextOverflow.ellipsis),
    ])),
...
class _Form extends StatefulWidget {
  ...
  const _Form({this.category, required this.onSaved});
...
class _FormState extends State<_Form> {
  ...
  @override void initState() { super.initState(); final c = widget.category; _name = TextEditingController(text: c?['name']); _desc = TextEditingController(text: c?['description']); }
  ...
  Future<void> _save() async {
    ...
    try {
      if (widget.category != null) await ApiService.put('/expense-categories/${widget.category!['id']}', body);
      else await ApiService.post('/expense-categories', body);
      widget.onSaved();
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(24, 16, 24, 32), child: Form(key: _form, child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(widget.category != null ? 'Edit Category' : 'Add Category', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700)), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]),
        ...
        TextFormField(controller: _name, decoration: const InputDecoration(labelText: 'Category Name *'), validator: (v) => v!.isNotEmpty ? null : 'Required'),
